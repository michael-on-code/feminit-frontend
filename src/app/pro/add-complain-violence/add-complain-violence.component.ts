...
@Component({
  selector: 'app-add-complain-violence',
  templateUrl: './add-complain-violence.component.html',
  styleUrls: ['./add-complain-violence.component.css']
})
export class AddComplainViolenceComponent implements OnInit {
  imageUrl: string;
  ...
  submit(form) {
    if (form.valid && this.imageUrl) {
      ...
      this.api.addComplainViolence(formData).subscribe((p: ApiReturnTypeService) => {
        const status = p.status ? 'success' : 'error';
        this.notification.create(status, 'Notification', p.message, {
          nzDuration: 10000
        });
        if (p.status) {
          this.route.navigate(['/dashboard/complaints/category/violences/edit/' + p.data]);
        }
